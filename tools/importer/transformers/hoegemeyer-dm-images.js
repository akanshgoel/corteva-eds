...
/**
 * Transformer: hoegemeyer Dynamic Media / Scene7 images.
 *
 * Source uses Scene7 IS/Image URLs on a vanity domain
 * (assets.vylor.com/is/image/pioneer/...). See migration-work/metadata.json
 * .images.mapping for the originals (cleaned.html shows localized hashed
 * paths for downloaded copies, but DM URLs such as the hero teaser image
 * remain inline as absolute URLs).
 *
 * Rewrites DM <img> tags into anchors so the URL round-trips through markdown
 * intact. The companion auto-block in scripts/scripts.js rebuilds responsive
 * <picture> markup from these anchors at render time.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into their cells; rewriting imgs to anchors earlier would
 * leave parsers with empty image cells.
 *
 * Canonical helpers below copied byte-identically from
 * references/dm-scene7-helpers.js (the subset the transformer consumes).
 */
